...
# 使用したデータについて
### 以下の説明変数は、架電後に得られるデータと考え、除外した
- day_of_week(最終連絡曜日)
- duration(通話時間(秒))
- campaign(キャンペーン期間中の接触回数)
- y(テレマーケティングの結果)

### また、以下の説明変数は、個人に紐づかないデータと考え、ペルソナの分析からは除外した
- emp.var.rate
- cons.price.idx(消費者物価指数)
- cons.conf.idx(消費者信頼感指数)
- euribor3m(3か月ユーリボー指標金利)
- nr.employed(四半期ごとの就業者数)

# 分析アプローチ
### 分析結果1.ターゲットとすべきペルソナについては、Rでロジスティック回帰モデルを使って分析する
### 分析結果2.予測精度、期待される収益については、Pythonで複数の機械学習モデルを使い分析する
